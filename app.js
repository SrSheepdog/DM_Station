...
function parseDice(expr){ // simple "XdY+Z" parser with support for multiple terms: 2d6+1+d4-2
  const tokens = expr.replace(/\s+/g,'').match(/([+-]?[^+-]+)/g);
  if(!tokens) return null;
  return tokens.map(t=>t.trim());
}
function rollTerm(term){ // returns {text, total}
  const m = term.match(/^([+-])?(\d*)d(\d+)|([+-]?\d+)$/i);
  if(!m) return {text:term,total:0};
  if(m[4]!==undefined){ return {text:m[4], total: parseInt(m[4],10)}; }
  const sign = (m[1]==='-')?-1:1; const count = m[2]?parseInt(m[2],10):1; const sides=parseInt(m[3],10);
  const rolls=[]; let sum=0; for(let i=0;i<count;i++){ const r=Math.floor(Math.random()*sides)+1; rolls.push(r); sum+=r; }
  return {text:`${sign<0?'-':''}${count}d${sides}(${rolls.join(',')})`, total: sign*sum};
}
function doRoll(expr){
  const parts = parseDice(expr||'');
  if(!parts) return {text:'Invalid roll', total:0};
  let total=0; const pieces=[];
  for(const p of parts){ const t=rollTerm(p); pieces.push(t.text); total+=t.total; }
  const out = `${expr} = ${pieces.join(' + ')} = **${total}**`;
  const hist = JSON.parse(localStorage.getItem(ROLL_KEY)||'[]'); hist.unshift({expr, out, at:new Date().toISOString()}); localStorage.setItem(ROLL_KEY, JSON.stringify(hist.slice(0,100)));
  return out;
}
...
function renderCard(e){
  ...
  const url = e.url?`<div class="small">${safe(e.url)}</div>`:'';
  return `<div class="card"><div class="badge">${safe(e.type)}</div><h4>${safe(e.title)}</h4>${url}<p>${safe(e.body)}</p></div>`;
}
