...
// ---------- pinboard ----------
const PIN_KEY='dmstation_pins_reset';
...
function buildEncounter(list,b){ const pool=list.slice().sort((a,b)=>a.xp-b.xp); const out=[]; let xp=0; const maxN=b<300?3:6; while(out.length<maxN && xp<b){ let pick=-1; for(let i=pool.length-1;i>=0;i--){ if(xp+pool[i].xp<=b){ pick=i; break; } } if(pick===-1) pick=0; out.push(pool[pick]); xp+=pool[pick].xp; } return {monsters:out,totalXP:xp}; }
...
async function loadSRD2014(){ const base='https://api.open5e.com';
  try{
    ...
    const pack=[]; spells.forEach(s=>{ const level=(s.level??s.level_int??'').toString(); const school=(s.school&&(s.school.name||s.school))||''; const text=[s.desc,s.higher_level,s.range_text,s.duration,s.components,s.casting_time].filter(Boolean).join('\n\n'); pack.push({type:'spell', title:s.name, body:`Level ${level} ${school}\n\n${text}`, url:s.url||''}); });
...
// ---------- Dice ----------
const ROLL_KEY='dmstation_rolls_reset';
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  $('btn-export-enc-roll20').addEventListener('click', async()=>{ const mons=await loadMon(); const size=clamp(parseInt($('enc-party-size').value||'4',10),1,8); const lvl=clamp(parseInt($('enc-party-level').value||'3',10),1,20); const diff=$('enc-difficulty').value; const filt=$('enc-filter').value; const b=budget(size,lvl,diff); const enc=buildEncounter(filterMon(mons,filt),b); download('encounter-roll20.txt', `Encounter (Total XP ${enc.totalXP})\n` + enc.monsters.map(m=>`- ${m.name} (CR ${m.cr}, ${m.xp} XP)`).join('\n')); });
  $('btn-export-enc-foundry').addEventListener('click', async()=>{ const mons=await loadMon(); const size=clamp(parseInt($('enc-party-size').value||'4',10),1,8); const lvl=clamp(parseInt($('enc-party-level').value||'3',10),1,20); const diff=$('enc-difficulty').value; const filt=$('enc-filter').value; const b=budget(size,lvl,diff); const enc=buildEncounter(filterMon(mons,filt),b); download('encounter-foundry.json', JSON.stringify({type:'encounter', totalXP:enc.totalXP, monsters:enc.monsters}, null, 2)); });

  // Quest
  $('btn-generate-quest').addEventListener('click',()=>{ const tone=$('quest-tone').value; const theme=$('quest-theme').value; $('quest-output').textContent=genQuests(tone,theme,3).join('\n\n'); });
  ...
  // Dice
  $('btn-roll').addEventListener('click',()=>{ const expr=$('dice-input').value||'1d20'; doRoll(expr); renderRolls(); });
